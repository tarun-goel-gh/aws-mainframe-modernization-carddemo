...
description: Govern and execute AWS Transform mainframe reverse engineering end to end — verify connectivity and account setup, intake COBOL/JCL source from GitHub, an S3 zip, or the local workspace, run discovery (analyze code, analyze data, discover data paths, discover business functions), then extract business logic and generate modernization requirements for every discovered business function, producing one verified spec zip per function plus a status report. Use when reverse engineering, assessing, decomposing, or reimagining a mainframe, COBOL, JCL, CICS, or VSAM codebase with AWS Transform.
compatibility: Requires the aws-transform Kiro power (MCP server) with AWS credential or SSO auth, AWS CLI v2, an AWS Transform mainframe reimagine connector (S3 + Amazon Neptune), and unzip plus python3 on PATH.
license: Apache-2.0
metadata:
  author: tarun.goel
  version: "1.1.1"
