...
**Note for this document specifically:** `atx-quality` is the weakest plan in this port. There is
no static-scan equivalent to Bob's Z Code Scan, and ISO 5055 scoring, CVE mapping and portfolio
roll-up were CAST capabilities. Expect more `Not available from AWS Transform analysis` markers
here than in any other document, and record each one in the manifest's `notApplicable` list
rather than substituting a plausible narrative.
