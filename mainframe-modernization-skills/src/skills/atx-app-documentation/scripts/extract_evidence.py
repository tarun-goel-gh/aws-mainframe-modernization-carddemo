...
for r in csv.DictReader(open(f"{M}/discovery/business_function.csv", newline="", encoding="utf-8")):
    ...
    if not n:
        continue
    # Read defensively. AWS Transform's catalog CSV has already varied between runs of this project,
    # and hard indexing turned a missing optional column into a KeyError traceback from step 5 of the
    # pipeline — a failure that says nothing about what is actually wrong. An absent column is a
    # documentable gap, not a crash.
    def _int(col):
        try:
            return int((r.get(col) or "").strip())
        except (TypeError, ValueError):
            return None

    pack["functions"][n] = {
        "description": (r.get("Business descriptions") or "").strip(),
        "dataPaths": _int("Number of data paths"),
        "entryPoints": _int("Number of entry points"),
        "loc": _int("Total LOC"),
        "missingFiles": _int("Number of potential missing files"),
        "entryPointList": [e.strip() for e in (r.get("List of entry points") or "").split(";")
                           if e.strip()],
        "slug": n.replace(" ", ""),
    }
