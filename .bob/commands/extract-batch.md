---
description: Run the cobol-knowledge-extraction skill against a project, folder, file list, inventory CSV, or glob
argument-hint: <path|file-list|inventory.csv|glob> [--dry-run]
---
Run the `cobol-knowledge-extraction` skill against this scope: $1

Resolve the scope per the skill's Step 1 — treat the argument as an inventory CSV, an explicit file
...
Finish with the skill's Step 15 report — coverage as done/total plus a percentage, and a named list of
